...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.myolivero.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-myolivero-custom-color', changedInput.value);
    inputToSync.setAttribute('data-myolivero-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-myolivero-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-myolivero-custom-color');
        });
    }
  }
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-myolivero-custom-color',
      textInput.getAttribute('data-myolivero-custom-color'),
    );
   ...
   */
  Drupal.behaviors.myoliveroColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'myolivero-color-picker',
        '[data-drupal-selector="edit-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'myolivero-color-picker',
        '[data-drupal-selector="myolivero-color-picker"] input[type="text"]',
      );
